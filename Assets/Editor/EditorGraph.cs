        ...
        for (int chan = 0; chan < Graph.MAX_CHANNELS; chan++)
        {
            ...
            for (int h = 0; h < Graph.MAX_HISTORY; h++)
            {
                ...
                if (xPix >= 0)
                {
                    ...
                    float y_01 = 1-Mathf.InverseLerp(Graph.YMin, Graph.YMax, y);

                    int yPix = (int)(y_01 * H);

                    Plot(xPix, yPix);
                    if(h != 0 && C.line) Line(xPix, yPix, (W - 1) - (h-1), (int)((1-Mathf.InverseLerp(Graph.YMin, Graph.YMax, C._data[h-1]))*H));
                    else Line(xPix, yPix, xPix, (int)((1-Mathf.InverseLerp(Graph.YMin, Graph.YMax, 0))*H));
                }
            }
        }
